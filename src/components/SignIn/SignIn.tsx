...
export default function SignIn() {
  ...
  const { user, setUser, loginVisible, setLoginVisible } = useAuth();
  ...
  const onSubmit = async (data: {email: string, password: string, tgid: string}) => {
    console.log(data);
    try {
      setErrorAuth(false);
      // запрос API
      const res:User = await getData(
        `http://${HOST}:${PORT}/connect`, data.email, data.password, data.tgid
      )
      if (res) {
        console.log(res);
  ...
  return (
    <Dialog
      ...
      header='Вход'
      modal
      onHide={() => {if (!loginVisible) return; setLoginVisible(false); }}
      content={({ hide }) => (
        <form
          autoComplete='none'
        ...
        >
          <div
            className={classNames('flex flex-column px-5 py-5 gap-2')}
            style={{
              borderRadius: '5px',
              backgroundImage: 'radial-gradient(circle at left top, var(--gray-900), var(--tg-theme-secondary-bg-color))'
            }}>
            ...
            { errorAuth && <small className='p-error'>Ошибка входа</small> }
            <div className='inline-flex flex-column gap-2 mt-4 mb-4'>
              
              <div className='field mb-2'>
                <span className='p-float-label p-input-icon-right'>
                  <Controller name='email'
                    control={control}
                    rules={{ required: 'Электронная почта обязательна.', pattern: { value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i, message: 'Неправильный адрес электронной почты. Например, example@email.com' }}}
                  ...
                  <label htmlFor='email' className={classNames({ 'p-error': !!errors.email })}>Почта*</label>
                </span>
                {errors.email && <small className="p-error">Электронная почта обязательна</small>}
              </div>
            </div>
            <div className='inline-flex flex-column gap-2'>
              ...
              <div className='field mb-4'>
                ...
                {  errors.password && <small className='p-error'>Пароль обязателен</small> }
              </div>
            </div>
            <div className='inline-flex flex-column gap-2 mt-4 mb-4'>
              ...
              <div className='field mb-2'>
                <span className='p-float-label p-input-icon-right'>
                  <Controller name='tgid'
                    control={control}
                    rules={{ required: 'Идентификатор обязателен.'}}
                    render={({ field, fieldState }) => (
                      <InputText
                        id={field.name}
                        autoComplete='off'
                  ...
                  <label htmlFor='tgid' className={classNames({ 'p-error': !!errors.tgid })}>id*</label>
                </span>
                {errors.tgid && <small className="p-error">Идентификатор обязателен.</small>}
              </div>
            </div>
